...
public class AMapUtil {
	...
	public static String getFriendlyLength(int lenMeter) {
		if (lenMeter > 10000) // 10 km
		{
			...
			return dis + com.example.map.ChString.Kilometer;
		}
		...
		if (lenMeter > 1000) {
			...
			return dstr + com.example.map.ChString.Kilometer;
		}

		if (lenMeter > 100) {
			int dis = lenMeter / 50 * 50;
			return dis + com.example.map.ChString.Meter;
		}
	...
	public static ArrayList<LatLng> convertArrList(List<LatLonPoint> shapes) {
		...
		for (LatLonPoint point : shapes) {
			LatLng latLngTemp = com.example.map.AMapUtil.convertToLatLng(point);
